...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      async function rq(e10) {
        let { pathname: t10 } = e10.nextUrl, r2 = (e11) => (e11.headers.set("Alt-Svc", "clear"), e11.headers.set("Cache-Control", "no-cache, must-revalidate"), e11);
        if (t10.startsWith("/api/auth/login")) return r2(el.next());
        let n2 = e10.cookies.get("pms_token")?.value;
        if (!n2) return r2(t10.startsWith("/api/") ? el.json({ error: "Unauthorized" }, { status: 401 }) : el.redirect(new URL("/login", e10.url)));
        let i2 = await rU(n2);
        if (!i2) {
          if (t10.startsWith("/api/")) return r2(el.json({ error: "Token expired or invalid" }, { status: 401 }));
          let n3 = el.redirect(new URL("/login", e10.url));
          return n3.cookies.set("pms_token", "", { maxAge: 0 }), r2(n3);
        }
        let a2 = new Headers(e10.headers);
        return a2.set("x-user-id", i2.sub), a2.set("x-user-role", i2.role), a2.set("x-user-email", i2.email), r2(el.next({ request: { headers: a2 } }));
      }
      e.s(["config", 0, { matcher: ["/((?!api/auth/login|api/debug|_next/static|_next/image|icon.png|login).*)"] }, "middleware", 0, rq], 99446);
...
var init_edgeFunctionHandler = __esm({
  "node_modules/@opennextjs/aws/dist/core/edgeFunctionHandler.js"() {
    ...
    globalThis._ROUTES = [{ "name": "middleware", "page": "/", "regex": ["^(?:\\/(_next\\/data\\/[^/]{1,}))?(?:\\/((?!api\\/auth\\/login|api\\/debug|_next\\/static|_next\\/image|icon.png|login).*))(\\\\.json)?[\\/#\\?]?$"] }];
